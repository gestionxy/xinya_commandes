...
def render_client_page():
    ...
    def _rm(i): 
        st.session_state.custom_data.pop(i, None); _reindex()
    ...
    if submit:
        ...
        chosen=[]
        for p in products:
            sel=st.session_state.get(f"sel_{p['id']}"); u=int(st.session_state.get(f"qtyu_{p['id']}",0)); c=int(st.session_state.get(f"qtyc_{p['id']}",0)); r=st.session_state.get(f"rem_{p['id']}","")
            if sel:
        ...
        for ce in customs:
            u=int(ce.get("qty_units",0) or 0); c=int(ce.get("qty_cases",0) or 0); has_img=bool(ce.get("img_bytes")); has_note=bool(str(ce.get("remark"," ")).strip())
            if u>0 or c>0:
        ...
        for idx,it in enumerate(chosen):
            if it.get("img_bytes"):
                ...
                norm = normalize_image_bytes(it["img_bytes"], 800, 600)
                Path(fname).write_bytes(norm)
        ...
        try:
            send_email_with_attachment(f"Xinya_Commandes_{order_id}", f"Bonjour {name},\n\nVotre commande est créée (ID: {order_id}). Le PDF est en pièce jointe.", [ADMIN_EMAIL, em], [str(pdf_path)])
